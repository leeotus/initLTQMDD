...
dd::Edge QuantumComputation::reduceAncillae(dd::Edge& e, std::unique_ptr<dd::Package>& dd, bool regular) {
	...
	dd::Edge QuantumComputation::buildFunctionality(std::unique_ptr<dd::Package>& dd) {
		static long long sth = 1000;
		static long long previousSize = 0;
		if (nqubits + nancillae == 0)
			return dd->DDone;
		...
		// 原始变量映射
		qc::permutationMap varMap = this->initialLayout;

		for (auto & op : ops) {
			auto tmp = dd->multiply(op->getDD(dd, line, this->initialLayout), e);
	...
	//linear sifting动态最小化
	dd::Edge QuantumComputation::buildFunDynLinSift(std::unique_ptr<dd::Package>& dd, permutationMap& theMap) {
		if (nqubits + nancillae == 0)
			return dd->DDone;
	...
	//sifting动态最小化
	dd::Edge QuantumComputation::buildFunDynSift(std::unique_ptr<dd::Package>& dd, permutationMap& theMap) {
		if (nqubits + nancillae == 0)
			return dd->DDone;
		...
		for (auto & op : ops) {
			...
			dd->incRef(newDD);
			// std::clog<<dd->size(newDD)<<std::endl;
