...
namespace dd {
	...
	uint64_t Package::computeLowerBoundDown(std::map<uint16_t, uint16_t>& varMap, uint16_t i) {
		...
		lb = labA + std::max((uint64_t)active.at(varMap[i]), 1+labB/NEDGE);
		return lb;
	}
	...
	uint64_t Package::computeLowerBoundUp(std::map<uint16_t, uint16_t>& varMap, uint16_t i) {
		...
		lb = labA + 1+active.at(varMap[i])/NEDGE + labC;
		return lb;
	}
	...
	uint64_t Package::computeTightLowerBoundDown(std::map<uint16_t, uint16_t>& varMap, uint16_t i, Edge in) {
		...
		// 方法1：原始 bound —— max(active[i], 1 + fixedBelow/NEDGE)
		uint64_t originalBound = std::max((uint64_t)active.at(varMap[i]), (uint64_t)(1 + fixedBelow / NEDGE));
		
		// 方法2：基于子节点共享的 bound
		uint64_t distinctChildren = countDistinctChildren(varMap[i]);
		uint64_t childBound = (distinctChildren + NEDGE - 1) / NEDGE;
		
		// 方法3：基于唯一出边模式数的 bound
		// countMaxNewNodes 统计交换后新层中不可合并的边模式数
		uint64_t patternBound = 0;
		if (i > 0) {
			patternBound = countMaxNewNodes(varMap[i]);
		}
	...
	uint64_t Package::computeTightLowerBoundUp(std::map<uint16_t, uint16_t>& varMap, uint16_t i, Edge in) {
		...
		// 原始 bound
		uint64_t originalBound = 1 + (uint64_t)active.at(varMap[i]) / NEDGE;
		
		// 基于子节点共享的 bound
		uint64_t childBound = ((uint64_t)active.at(varMap[i]) + NEDGE - 1) / NEDGE;
		
		// 基于边模式的 bound（向上移动时，考虑 level i+1 的边模式）
		uint64_t patternBound = 0;
		if (i + 1 < n) {
			patternBound = countMaxNewNodes(varMap[i + 1]);
		}
		
		uint64_t upperLayerBound = std::max(originalBound, std::max(childBound, patternBound));
	...
	void Package::printIndToVar(std::map<unsigned short, unsigned short> varMap) {
		...
		std::cout<<std::endl<<"index"<<':'<<"variable"<<std::endl;
	...
	void Package::printLTMap(std::map<unsigned short, unsigned short> varMap, bool Mat[MAXN][MAXN])
	{
		unsigned short nqubits = varMap.size();
		std::map<unsigned short, unsigned short> invVarMap{}; //DD qubit（变量） 到 电路 qubit（变量） 的映射
		for ( auto & i : varMap)
    ...
    void Package::xorLinear(unsigned short index,  std::map<unsigned short, unsigned short>& Map, bool Mat[MAXN][MAXN]) { //更新lt矩阵
        const unsigned short varNum = Map.size();
		for(int i=0; i<varNum; ++i) {
            Mat[Map[index]][i] ^= Mat[Map[index-1]][i];
		}
	...
	void Package::printOpSeq(std::list<Move> opSeq) {
		for( auto &i : opSeq) {
			if(i.optype == SWAP_MOVE)
				...
				std::cout << "lt:" << i.index << ", ";
			else if(i.optype == UPDATE_MAP)
				std::cout << "update map" << ", ";
			else
				throw std::logic_error("操作序列有问题 ");
		}
		std::cout << std::endl;
	}

	bool Package::matIsEqual(bool mat[MAXN][MAXN]) {
		for(int i=0; i<MAXN; ++i)
			for(int j=0; j<MAXN; ++j) {
				if(mat[i][j] != xorMat[i][j]) return false;
			}
		return true;
	}
