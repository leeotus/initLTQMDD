...
//		}
	...
	/// Sifting with externally specified variable processing order
    std::tuple<Edge, unsigned int, unsigned int> Package::sifting(Edge in, std::map<unsigned short, unsigned short>& varMap, const std::vector<short>& varOrder) {
		const auto n = static_cast<short>(in.p->v);
        ...
        for (int i = 0; i < n; ++i) {
            ...
            // Use external order: pick next variable from varOrder that is still free
            bool found = false;
            for (auto candidate : varOrder) {
                if (candidate >= 0 && candidate < n && free.at(varMap[candidate])) {
                    pos = candidate;
                    found = true;
                    break;
                }
            ...
            if (!found) {
                // Fallback: pick max active (same as standard sifting)
                unsigned long max = 0;
                for (short j = 0; j < n; j++) {
	...
	std::tuple<Edge, unsigned int, unsigned int> Package::lbSifting(Edge in, std::map<unsigned short, unsigned short>& varMap) {
		const auto n = static_cast<short>(in.p->v);
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (pos < n / 2) {  // variable is in lower half -> sifting to bottom first
                // sifting down with lower bound pruning
                while (pos > 0) {
                ...
                // sifting up with lower bound pruning
                while (pos < n - 1) {
